...
function BlogPost() {
  ...
  const renderContent = () => {
    ...
    if (post.content.includes("<h2") || post.content.includes("<strong>")) {
      return <div className="prose prose-invert max-w-none prose-brand text-muted-foreground leading-loose" dangerouslySetInnerHTML={{ __html: post.content }} />;
    }
      ...
      <div className="text-lg text-muted-foreground leading-relaxed space-y-6">
        {post.content.split("\n\n").map((paragraph, i) => {
          if (paragraph.startsWith("# ")) {
             return <h2 key={i} className="text-3xl font-bold text-foreground mt-12 mb-6">{paragraph.replace("# ", "")}</h2>;
          }
          if (paragraph.startsWith("## ")) {
            return <h3 key={i} className="text-2xl font-bold text-foreground mt-10 mb-4">{paragraph.replace("## ", "")}</h3>;
         }
      ...
      <main className="max-w-4xl mx-auto px-6 mt-12 md:mt-20">
        ...
        <header className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            <span className="text-sm text-muted-foreground flex items-center gap-2">
              {post.date ? new Date(post.date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : "Post recente"}
            </span>
          </div>
        ...
        <article className="max-w-3xl mx-auto">
          ...
          {post.link && post.link !== "#" && (
            <div className="mt-12 pt-6 border-t border-border/50">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed break-all">
                AUTOR DESCONHECIDO. <strong>{post.title}</strong>. Disponível em: &lt;<a href={post.link} target="_blank" rel="noreferrer" className="text-brand hover:underline">{post.link}</a>&gt;. Acesso em: {post.date ? new Date(post.date).toLocaleDateString('pt-BR') : new Date().toLocaleDateString('pt-BR')}.
              </p>
            </div>
          )}
          ...
          <div className="mt-8 p-4 border-l-2 border-muted-foreground/30 bg-muted/20 text-muted-foreground text-sm flex items-start gap-3 rounded-r-lg">
            ...
            <p>
              <strong>Transparência de Conteúdo:</strong> Esta reportagem foi rastreada, lida e roteirizada de forma 100% autônoma por nossos <em>Agentes de Inteligência Artificial</em>. A automação no centro da informação.
            </p>
          </div>
          ...
          <div className="mt-20 p-8 rounded-2xl bg-brand/5 border border-brand/20 text-center relative overflow-hidden">
            ...
            <h3 className="text-2xl font-bold text-foreground mb-4 relative z-10">Sua empresa ainda perde dinheiro com operações manuais?</h3>
            <p className="text-muted-foreground mb-8 relative z-10">A <strong>Martines Produtos Digitais</strong> cria hiperautomações inteligentes que acabam com a ineficiência. Agende uma consultoria e veja a IA trabalhar para você.</p>
            <a href="https://wa.me/5511954210088" target="_blank" rel="noreferrer" className="relative z-10 inline-flex h-12 items-center justify-center rounded-md bg-brand px-8 text-sm font-medium text-primary-foreground shadow transition-colors hover:bg-brand/90">
