...
import { Shield, ChevronRight, Settings, Webhook, Activity, LayoutDashboard, Cpu, HardDrive, Server, Folder, ShieldCheck, BarChart3, Trash2, FileText } from 'lucide-react';
...
function AreaRestrita() {
  ...
  const handleDeletePost = async (slug: string) => {
    if (confirm("Tem certeza que deseja apagar essa reportagem? Ela sumirá do site imediatamente.")) {
    ...
    <div className="min-h-screen bg-[#0B1120] text-white flex font-sans">
      ...
      <aside className="w-64 bg-[#111827] border-r border-[#1F2937] flex flex-col hidden md:flex">
        ...
        <div className="p-4 border-t border-[#1F2937]">
          <a href="/" className="w-full flex items-center gap-3 text-gray-400 hover:text-white px-4 py-3 rounded-lg hover:bg-[#1F2937] transition-colors">
      ...
      <main className="flex-1 p-6 lg:p-10 overflow-y-auto">
        <div className="max-w-6xl mx-auto space-y-8 animate-fade-up">
          ...
          <div>
            ...
            <p className="text-gray-400 mt-1">Aqui está o resumo da sua infraestrutura digital e monitoramento de tráfego.</p>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-6">
                <h3 className="text-sm font-medium text-gray-400 mb-6">Visitas Diárias (Últimos 30 dias)</h3>
              ...
              <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-6">
                <h3 className="text-sm font-medium text-gray-400 mb-6">Pico de Acessos por Horário (24h)</h3>
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={hourlyData}>
                      ...
                      />
                      <Line type="monotone" dataKey="acessos" stroke="#3B82F6" strokeWidth={3} dot={{ r: 4, fill: '#3B82F6', strokeWidth: 0 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-6">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-400">rdo-app</span><span>310.1MiB</span></div>
                <div className="flex justify-between"><span className="text-gray-400">rdo-mobile</span><span>4.977MiB</span></div>
                <div className="flex justify-between"><span className="text-gray-400">n8n</span><span>349.5MiB</span></div>
                <div className="flex justify-between"><span className="text-gray-400">evolution-api</span><span>252.0MiB</span></div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            {activeFolder === null ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <a href="https://n8n.martines.halftech.com" target="_blank" rel="noreferrer" className="bg-[#111827] border border-[#1F2937] hover:border-red-500/50 transition-colors rounded-xl p-6 flex flex-col items-center text-center group">
                ...
                <a href="http://216.22.43.39:81/" target="_blank" rel="noreferrer" className="bg-[#111827] border border-[#1F2937] hover:border-blue-500/50 transition-colors rounded-xl p-6 flex flex-col items-center text-center group">
                ...
                <a href="https://api.martines.halftech.com/manager" target="_blank" rel="noreferrer" className="bg-[#111827] border border-[#1F2937] hover:border-green-500/50 transition-colors rounded-xl p-6 flex flex-col items-center text-center group">
                ...
                <button onClick={() => setActiveFolder('projetos')} className="bg-[#111827] border border-[#1F2937] hover:border-yellow-500/50 transition-colors rounded-xl p-6 flex flex-col items-center text-center group">
              ...
              <div className="animate-fade-in">
                <button onClick={() => setActiveFolder(null)} className="mb-4 text-sm text-gray-400 hover:text-white flex items-center gap-1 transition-colors">
                  <ChevronRight className="h-4 w-4 rotate-180" /> Voltar para Ferramentas
                </button>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <a href="http://216.22.43.39:8081/" target="_blank" rel="noreferrer" className="bg-[#111827] border border-[#1F2937] hover:border-cyan-500/50 transition-colors rounded-xl p-6 flex flex-col items-center text-center group">
                  ...
                  <a href="http://216.22.43.39:8082/" target="_blank" rel="noreferrer" className="bg-[#111827] border border-[#1F2937] hover:border-purple-500/50 transition-colors rounded-xl p-6 flex flex-col items-center text-center group">
                      ...
                      <tr key={i} className="border-b border-[#1F2937] hover:bg-[#1F2937]/50 transition-colors">
                        <td className="px-6 py-4 font-medium text-white max-w-xs">
                          <div className="flex items-center gap-3">
                            {post.image ? (
                              <img src={post.image} alt="Capa" className="w-10 h-10 rounded object-cover border border-[#1F2937] flex-shrink-0" />
                            ) : (
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          {post.date ? new Date(post.date).toLocaleDateString('pt-BR') : 'Data Indisponível'}
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <button 
                            onClick={() => post.slug ? handleDeletePost(post.slug) : alert("Posts estáticos ou sem slug não podem ser deletados.")}
